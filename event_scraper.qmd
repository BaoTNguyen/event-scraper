...
# STILL WORKING ON LOADING TRANSFORMER MODEL
# reticulate::py_install(
#   packages = c("spacy-transformers", "torch"),
#   envname = "r-spacyr",
#   pip = TRUE
# )
...
# ")
# spacy_download_langmodel(lang_models = "en_core_web_trf")
...
# Clean up entities and output important entities
entities_tbl <- ner_df %>%
  filter(ent_type %in% keep_types) %>%
  unnest_tokens(output = token, input = text) %>%                     # split entity into tokens
  anti_join(stop_words, by = c("token" = "word")) %>%  # remove stop words
  group_by(doc_id, ent_type, length) %>%
  summarise(
    entity_clean = str_c(token, collapse = " "),
    length = n(),
    .groups = "drop"
  ) %>%
  filter(entity_clean != "") %>%                       # drop any empty results
  left_join(entity_category_map, join_by("ent_type" == "entity_type")) %>%
  distinct(doc_id, category, entity_clean) %>%
  group_by(doc_id, category) %>%
  summarise(entities = list(entity_clean), .groups = "drop")
