...
The data is wrangled using a javascript web scraper. Key features are extracted such as Title, Description, and more. An example output is provided here:
...
We also face a problem of non-descriptive phrases containing filler words not already stopped out. Differentiating contexts for n-grams \>1 can prove quite difficult. To side-step both these problems, and enhance our analysis, we can rely on our enriched data frame to pull noun-phrases to evaluate trends.
...
We can observe some clustering of events allowing us to establish some inter-connectivity if any. We can further refine our uni-gram and bi-gram matching to include only events with relevant words. We utilize lexicons such as the one below as a comparative list.
