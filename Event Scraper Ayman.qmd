...
```{r} node_bin <- Sys.which("node") if (node_bin == "") {   stop("Node is not available on this system; install it to run the scraper.") }  script_path <- file.path("platformcalgary.js") cmd <- sprintf('"%s" "%s"', node_bin, script_path) scrape_raw <- system(cmd, intern = TRUE)  # Parse JSON output from the scraper into an R data frame events_df <- jsonlite::fromJSON(paste(scrape_raw, collapse = "\n")) print(events_df)}
```
